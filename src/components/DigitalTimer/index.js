...
class Calculator extends Component {
  ...
  equalsButtonClicked = () => {
    ...
    this.setState(prevState => ({
      ...
      divideEqualsCount: prevState.divideEqualsCount + 1,
    }))
    const {intoEqualsCount, divideEqualsCount} = this.state
    if (sign === '+') {
      this.setState(prevState => ({
        firstValue: prevState.firstValue + parseFloat(ans),
      ...
      if (divideEqualsCount === 0) {
        const {firstValue} = this.state
        this.setState(prevState => ({
          firstValue: prevState.firstValue / parseFloat(ans),
          isFunctionStarted: true,
          ans: firstValue,
          equalsDivideValue: firstValue,
          divideEqualsCount: prevState.divideEqualsCount + 1,
        }))
      } else {
        const {firstValue, equalsDivideValue} = this.state
        this.setState(prevState => ({
          firstValue:
            (prevState.firstValue / parseFloat(ans)) * equalsDivideValue,
          isFunctionStarted: true,
          ans: firstValue,
          equalsDivideValue: firstValue,
        }))
